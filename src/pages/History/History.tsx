...
function History() {
  ...
  useEffect(() => {
    ...
    body?.addEventListener("click", () => {
        setDisplayFilterOptions(false)
    })
  }, [])

  const isSearching = Boolean(searchInput || transactionType != "all")
  ...
  return (
    <div className='text-[#191919] trackin-[0%] leading-[150%] pt-8'>
        ...
        <section className=" flex gap-4 px-6 mb-3">
            ...
            <div className="relative min-w-21.75 w-21.75 h-9.25 flex gap-1.5 border border-[#E1E3ED] items-center justify-center rounded-sm text-sm cursor-pointer" onClick={(e) => {
                e.stopPropagation()
        ...
        {<section className=''>
            ...
            // display ths part when a user is searching or filtering transactions
            <div className='px-6 pt-3'>
                {
                    Object.values(transactionHistory).flat().filter(y => transactionType === "received" ? y.type === "topup" : transactionType === "sent" ? y.type === "transfer" : y).filter(x => searchInput.length > 0 ? x.receiver.name.toLowerCase().includes(searchInput.toLocaleLowerCase()) : x).map((transaction) => (
                        <div className='flex mb-3 justify-between border-b border-[#EDEFF6] pb-3 pt-1 last:border-b-0 cursor-pointer' key={transaction.id} id={transaction.id} onClick={() => {
            ...
            // display this part when a user is not filtering through transactions
            <section className=''>
            {
                    ...
                    <div className={`px-6 border-[#EDEFF6] last:border-b-0 pt-3 ${(transactionType !== "all") ?  "" : searchInput.length > 0 ? "" : "border-b-[6px]"}`} key={sectionName}>
                        ...
                        
                        {
                            transactions.map(transaction => (
                                <div className='flex mb-3 justify-between border-b border-[#EDEFF6] cursor-pointer pb-3 pt-1 last:border-b-0' key={transaction.id} id={transaction.id} onClick={() => {
                                    setDisplayModal(true)
                                    setSelectedTx({
                                            img: transaction.receiver.img,
                                            name: transaction.receiver.name,
